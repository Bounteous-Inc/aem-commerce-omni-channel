...
/**
 * Import product content from a CSV and generate respective AEM Mobile Project content
 */
public class Importer {
    ...
    private void importCSVRow(CSVRecord csvRecord) throws Exception {
        try {
            ...
            String imagePath = "/content/dam/adobe-cares" + getString(csvRecord.get(9));
            ...
            String collectionPath = appNode.getPath() + "/collections/" + collectionTitle;
            if (!session.nodeExists(collectionPath)) {
                Node collectionsParentNode = getFolderNodeForGeneratedContent(appNode, "collections");
                collectionNode = collectionsParentNode.addNode(collectionTitle, "cq:Page");
                ...
                ContentCreationUtil.updateCollection(collectionContentNode, collectionTitle, imagePath, getLayout());
            } else {
            ...
            String entityPath = appNode.getPath() + "/" + parentNodeName + "/" + entityName;
            String md5 = Base64.encodeBase64String(DigestUtils.md5(new StringBufferInputStream(csvRecord.toString())));

            if (!session.nodeExists(entityPath)) {
                Node entityParentNode = getFolderNodeForGeneratedContent(appNode, parentNodeName);
                entityNode = entityParentNode.addNode(entityName, "cq:Page");
                LOGGER.info("CREATED: " + entityPath);
                Node entityContentNode = entityNode.addNode("jcr:content", "cq:PageContent");
                if (isBanner) {
                    ContentCreationUtil.updateBanner(entityContentNode, md5, shortTitle, collectionTitle, imagePath);
                } else {
                    ContentCreationUtil.updateArticle(entityContentNode, md5, shortTitle, collectionTitle, internalKeyword, title, description, price, feature1, feature2, feature3, imagePath);
                }
            } else {
    ...
    private Node getFolderNodeForGeneratedContent(Node appNode, String entityFolderName) throws RepositoryException {
        Node entityFolderNode = appNode.getNode(entityFolderName);
        return entityFolderNode;
        /** Don't use folders yet
         if(entityFolderNode.hasNode(NN_GENERATED)){
         return entityFolderNode.getNode(NN_GENERATED);
         } else {
         return entityFolderNode.addNode(NN_GENERATED, "sling:Folder");
         }
         */
    }
    ...
    private void resetNode(Node node) throws RepositoryException {
        ...
        while (propertyIterator.hasNext()) {
            ...
            if (!name.equals("dps-id") && !property.getDefinition().isProtected()) {
    ...
    private boolean isUpdated(Node articleContentNode, String md5) throws RepositoryException {
        if (articleContentNode.hasProperty("importMD5")) {
            return !md5.equals(articleContentNode.getProperty("importMD5").getString());
        } else {
            return true;
        }
    }
